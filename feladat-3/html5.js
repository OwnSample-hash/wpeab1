...
function getLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
            document.getElementById('locationData').innerText = `Lat: ${position.coords.latitude}, Lon: ${position.coords.longitude}`;
        });
    }
}
...
dragText.ondragstart = (event) => event.dataTransfer.setData("text", event.target.id);
